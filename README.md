...
2. Cấu hình các biến môi trường cần thiết (Cơ sở dữ liệu PostgreSQL và thông tin xác thực Supabase S3) trong tệp `application.yml`.
3. Chạy lệnh `mvn spring-boot:run` để khởi động máy chủ.
4. Truy cập ứng dụng tại địa chỉ `http://localhost:8080`.
...
2. Configure the necessary environment variables (PostgreSQL database and Supabase S3 credentials) in the `application.yml` file.
3. Execute the command `mvn spring-boot:run` to start the server.
